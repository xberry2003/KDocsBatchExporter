...
项目已完成正式收尾，当前进入维护和优化阶段。它用于扫描企业空间目录，并根据文件类型选择直接下载或官方 DOCX 导出链，同时支持失败重试、文件校验、断点恢复、manifest 记录和 Golden A/B 回归对比。

## 项目状态

- 当前版本为稳定交付版，后续只做兼容性修复、可靠性优化和小范围能力增强。
...
- 生产运行前建议先用 `--scan-only` 或小目录样本确认凭据、输出路径和人工处理项。
...
完结状态和维护说明见 [PROJECT_STATUS.md](PROJECT_STATUS.md)。
